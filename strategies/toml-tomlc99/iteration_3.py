...
# been measured to accept -- \x0d (bare CR, illegal outside a CRLF pair) was
# added this round to widen coverage of that same accepted-superset family.
...
def string_value(draw):
    kind = draw(st.sampled_from([
        ...
        "basic_control_byte", "basic_control_byte", "basic_invalid_escape",
    ]))
    if kind == "basic":
        ...
        # Grammar forbids this; tomlc99 is known to accept it. Given at
        # double weight now: this is the confirmed-real superset gap, not
        # the depth-based crash the loop already found repeatedly.
...
# stack-overflow bug. Four unique crash signatures have now been confirmed
# across runs (array nesting, inline-table nesting, and mixed variants), so
# per the loop's own guidance -- steer away from bugs already found -- this
# mode's share of the overall mix is trimmed further this round (10% -> 6%)
# rather than expanded. It stays alive only to keep the regression warm, not
# to keep re-finding the same class of crash.
# ---------------------------------------------------------------------------
...
def normal_document(draw):
    ...
    # handful of malformed-value edge cases folded in here so they ride along
    # inside otherwise well-formed documents instead of only ever appearing
    # as bare single-value probes. The denominator was tightened from 30 to
    # 24 this round -- the measured 39.4% overall acceptance rate has
    # headroom for more malformed-but-plausible input before it starts
    # testing the tokenizer instead of the parser.
    extra_choice = draw(st.integers(min_value=0, max_value=23))
    if extra_choice == 0:
        ...
        # unterminated inline table -- missing closing brace.
        extra = "zzunterm_tbl = {a = 1"
        doc = (doc + newline + extra) if doc else extra
    elif extra_choice == 8:
        # mismatched array-table close bracket (single ']' instead of ']]').
...
def toml_document_strategy(draw):
    # Four unique crash signatures have now been confirmed for the deep
    # nesting stress paths (array, inline-table, and mixed variants) --
    # per the loop's guidance, budget for already-found bug classes is
    # trimmed rather than grown, freeing weight for normal_document's
    # widened malformed-edge density and the string-level control-byte
    # superset gap, which is the confirmed-real (not merely hypothesized)
    # accept-when-should-reject family per ADAPTATIONS.md.
