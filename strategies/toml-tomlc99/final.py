# Final strategy: iteration 2, 4 unique crash signature(s).
# Verbatim copy of iteration_2.py; see logs/toml-tomlc99/ for how it evolved.
...
# Expanded beyond the original small sample: more raw control bytes across
# the C0 range plus the DEL/high-C1-adjacent byte, all of which the grammar
# forbids inside a basic string but tomlc99 has been measured to accept.
CONTROL_BYTES = ['\x01', '\x02', '\x03', '\x0b', '\x0c', '\x0e', '\x1f', '\x7f']
...
def string_value(draw):
    kind = draw(st.sampled_from([
        "basic", "basic", "basic_ml", "literal", "literal_ml",
        "basic_control_byte", "basic_invalid_escape",
    ]))
    if kind == "basic":
        ...
        # Kept at low frequency purely to keep the rejection path warm --
        # not a new bug lead, just coverage of an already-confirmed boundary.
...
# stay fast and well-formed; a separate mid-depth stress path below fills the
# gap the previous iteration's depth histogram exposed between st.recursive's
# shallow output (d0-d4) and the extreme stack-overflow probes (d500+), and
# deep-nesting stress remains isolated single-value documents further down).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mid-depth arrays / inline tables: the previous run's depth histogram
# (d0:77, d1:98, d2:124, d3:129, d4:34, then a hard jump straight to d500+)
# showed nothing was generated in the 5-499 range at all. That is a real gap:
# it is exactly the region where a C parser's recursion might show resource
# problems (deep but not stack-overflowing) that never show up either in the
# tiny st.recursive trees or in the already-crashing 500-35000 extreme probes.
# These are isolated single-value documents for the same reason the extreme
# ones are: so a slow parse never drags down the accept rate of an unrelated
...
# stack-overflow bug. These already produced confirmed crash signatures
# (array nesting and inline-table nesting both crashed last run), so their
# share of the overall document mix is intentionally kept modest now --
# enough to keep confirming the bug is still present, not so much that it
# starves the parts of the grammar that have not produced a crash yet.
# ---------------------------------------------------------------------------
...
def normal_document(draw):
    ...
    # syntactically valid per the grammar (which has no uniqueness
    # constraints) but known rejection points (duplicate key/table) or known
    # tomlc99 superset-acceptance (array-of-table redefined as key), plus a
    # couple of extra malformed-value edge cases folded in here so they ride
    # along inside otherwise well-formed documents instead of only ever
    # appearing as bare single-value probes. The denominator was tightened
    # from 60 to 30 (still low-frequency, but roughly double the previous
    # rate) since the measured 43% overall acceptance rate has headroom for
    # more malformed-but-plausible input without collapsing into pure
    # tokenizer-reject territory.
    extra_choice = draw(st.integers(min_value=0, max_value=29))
    if extra_choice == 0:
        ...
        # array with a trailing comma and no preceding value at all --
        # a bare comma inside otherwise-empty brackets.
        extra = "zzemptycomma = [,]"
        doc = (doc + newline + extra) if doc else extra
    elif extra_choice == 4:
        # inline table with a dangling trailing comma (grammar's
        # inline_table_keyvals_non_empty has no trailing-comma alternative).
        extra = "zztrail = {a = 1,}"
        doc = (doc + newline + extra) if doc else extra
    elif extra_choice == 5:
        # unterminated basic string -- should reject at the tokenizer.
...
def toml_document_strategy(draw):
    # Deep-stress modes already produced confirmed crash signatures for both
    # array and inline-table nesting last run (unique_crash_signatures=3),
    # so their combined weight is trimmed from 20% to 10% -- kept alive to
    # reconfirm the bug still fires, not to keep re-finding it. The freed
    # budget goes to the new mid-depth probes (filling the d5-d499 gap the
    # depth histogram exposed) and to normal_document, whose malformed-edge
    # density was also raised above.
